...
export const List = () => {
  ...
  onMount(async () => {
    ...
    if (locationRoute === 'articles' || locationRoute === 'slides') {
      ...
      const replacePathName = locationPath.substring(
        locationPath.indexOf('/'),
  ...
  return (
    ...
    >
      ...
      <Stack direction="column">
        <Show when={route() === 'articles'} fallback={<></>}>
          <Show
          ...
          <Show when={!data.loading && data()!.zenn! && data()!.zenn!.length > 0} fallback={<></>}>
          ...
          <Show when={!data.loading && data()!.note! && data()!.note!.length > 0} fallback={<></>}>
            <Box sx={{ paddingTop: '20px' }}></Box>
            <ListParts
              title={'Note'}
              id={'note'}
